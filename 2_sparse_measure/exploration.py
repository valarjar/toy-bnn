...
if __name__ == "__main__" :
	...
	parser.add_argument("--trainsetsize", type=int, default = 10000, help="Number of samples to generate in the training set.") #10000
	parser.add_argument("--trainsetobsprob", type=float, default = 0.2, help="probability of getting a measure for each item in an output vector for the train set.") # 0.2
	parser.add_argument("--testsetsize", type=int, default = 5000, help="Number of samples to generate in the test set.") # 5000
	...
	normTestInputs = trainInputs.T/np.std(trainInputs.T, axis=0, keepdims=True)
	normTestOutputs = trainOutputs.T/np.std(trainOutputs.T, axis=0, keepdims=True)
